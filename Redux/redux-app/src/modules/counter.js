...
function* decreaseSaga() {
    yield delay(2000); // 1초기다림
    yield put({type:"decrease"}) // 액션을 실행(disoatch) 
}
...
// 리덕스 사가를 실행하기위한 액션함수 >> saga도 payload를 통해 값을 가지고 올수 있다. 
...
function counter(state = initalState, action) {
    switch(action.type) {
        case "increase":
            ...
            console.log(num);
            return { ...state , number : state.number+num+100};
        case "decrease":
            return { ...state, number : state.number-state.changeNum-100}
        case "change":
